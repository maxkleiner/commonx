...
{$ENDIF}
  {$IFDEF USE_SPINLOCKS}
  FHeapLocks: array[0..MAXSIZEINDEX] of TSPinLock;
  {$ELSE}
  FHeapLocks: array[0..MAXSIZEINDEX] of TCLXCriticalSection;
  {$ENDIF}
...
function GetSizeIndexSizeFromBlockSizeInclHeader(const BlockSize: NativeInt): NativeInt;{$IFNDEF NOINLINE}inline;{$ENDIF}
function GetSizeIndexSize(const idx: byte): NativeInt;{$IFNDEF NOINLINE}inline;{$ENDIF}
function GetsizeIndex_Headincluded(const BlockSize: NativeInt): NativeInt;{$IFNDEF NOINLINE}inline;{$ENDIF}
function GetsizeIndex_Headless(const BlockSize: NativeInt): NativeInt;{$IFNDEF NOINLINE}inline;{$ENDIF}
...
begin
  ...
  if (not bLarge) then begin
    iNewSize := GetSizeIndexSizeFromBlockSize(iNEwSize);
  end;
  {$ELSE}
  iNewSize := GetSizeIndexSizeFromBlockSizeInclHeader(iNEwSize);
  {$ENDIF}
  {$IFNDEF USELITEASOS}
...
{$IFDEF LOCK_BLOCK_MAN}  InitializeCRiticalSectionAndSPinCount(sect, sys.dwNumberofProcessors*SPIN_COUNT);{$ENDIF}
  InitBlocks;
...
begin
  self.FinalizeBlocks;
{$IFDEF LOCK_BLOCK_MAN}  DeleteCriticalSEction(sect);{$ENDIF}
  inherited;
end;
...
{$IFDEF EXTRA_ERR_CHECKING}try{$ENDIF}
    iSizeIndex := GetInflatedIndex(UserSize);//  iSizeIndex := GetSizeIndex_HeadIncluded(InflateSize(UserSize));
...
var
  t: NativeInt;
begin
  for t:=0 to MAXSIZEINDEX do begin
    {$IFDEF USELISTCLASS}
...
{$ENDIF}
  ...
  iSizeIndex := GetSizeIndex_Headless(UserSize);
...
{ TSuperMemBlock }
function GetSizeIndexSizeFromBlockSizeInclHeader(const BlockSize: NativeInt): NativeInt;
begin
{$IFDEF FIB}
  result := GetSizeIndex_HeadIncluded(BlockSize);
  result := FIBS[result]{$IFDEF LESSWASTE}+SizeOf(TSuperMemBlock){$ENDIF};
{$ELSE}
...
{$IFDEF FIB}
  result := FIBS[idx];
{$ELSE}
  result := (1 shl idx)//{$IFDEF LESSWASTE}+SizeOf(TSuperMemBlock){$ENDIF};
//  result := 1 shl idx;
{$ENDIF}
end;
...
{$IFDEF FIB}
  var ni := BlockSize;
  for var t := 0 to high(FIBS) do begin
    var fibby := FIBS[t];
    if fibby >= ni then
        exit(t);
  end;
...
function GetSizeIndex_HeadIncluded(const BlockSize: NativeInt): NativeInt;
begin
{$IFDEF FIB}
  var ni := BlockSize{$IFDEF LESSWASTE}-SizeOf(TSuperMemBlock){$ENDIF};
  for var t := 0 to high(FIBS) do begin
    var fibby := FIBS[t];
    if fibby >= ni then
        exit(t);
...
//  Addr := @self;
  ...
  self.FNextBlock := nil;
  self.ManagedBy := nil;
  self.SizeIndex := GetSizeIndex_Headless(UserSize);
...
{$IFNDEF FIB}
  var ni := size +sizeof(TSuperMemBlock);
  ni := ni{$IFDEF LESSWASTE}-SizeOf(TSuperMemBlock){$ENDIF};
  result := (HighOrderBit(ni)+1);
{$ELSE}

//    result := (GetSizeIndexSizeFromBlockSizeInclHeader(ni));
    result := GetsizeIndex_Headless(size);
...
{$ENDIF}
{$IFDEF FIB}
    result := GetSizeIndexSize(self.SizeIndex){$IFDEF LESSWASTE}+SizeOf(TSuperMemBlock){$ENDIF};
{$ELSE}
...
begin
  {$IFDEF STATS}
  var uas := smb_GetUserAreaSize(self);
  {$IFDEF DONT_CACHE_LARGE_BLOCKS}
  ...
  var rub := self.ReportedUsedBytes;
  if rub > 0 then
    result := uas - rub
  else
...
function TBlockManager.Clean: nativeint;
...
{$IFDEF EXTRA_ERR_CHECKING}try{$ENDIF}
    cx := 0;
    for t:= MAXSIZEINDEX downto 0 do begin
...
function TThreadBlockManager.Clean: nativeint;
var
  t,u: NativeInt;
  block: PSuperMemBlock;
  cx: NativeInt;
...
function TThreadBlockManager.EmptyBlocksToMain: nativeint;
var
  t: NativeInt;
  block: PSuperMemBlock;
...
function  TBlockManagerManager.Clean: nativeint;
var
  t: NativeInt;
  man: TBlockManager;
//  tm1, tm2: cardinal;
begin
  ...
  {$IFDEF FREEMAN}{$IFDEF LIMIT_MANS}freeman not supported with limit_mans because manager could be borrowed{$ENDIF}{$ENDIF}
...
begin
  ...
  try
    ...
    //if we have too many managers, we will have to reuse one
    if (result = nil) then begin
{$IFDEF LIMIT_MANS}
    unstable
      if ManagerCount > MANAGER_LIMIT then begin
        result := Managers[random(managercount)-1] as TThreadBlockManager;
        borrowedManager := true;
      end else
{$ENDIF}
...
{x$DEFINE SIMPLER_FREE}
{$IFNDEF SIMPLER_FREE}
 while true do begin
  var block := PSuperMemBlock(PAnsiChar(p)-SizeOf(TSuperMemblock));
  var list := PLockedBlockList(block.ManagedBy);
//  man := list.Owner;
...
//    block.WasteBytes := 0;
  ...
  {$IFDEF STATS}TallyStats(block);{$EndIF}
{$IFDEF HOLD_MEMORY}
...
//    exit;
  ...
  {$IFDEF STATS}UnTallyStats(block);{$ENDIF}

  var nxt := PSuperMemBlock(block.FNextBlock);
  var prev := PSuperMemBlock(block.FPreviousBlock);

    var bHasPrior := block.FPreviousBlock <> nil;
    var bHasNext := block.FNextBlock <> nil;
  ...
  if bHasNext and (block = tip) then
    tip := block.FNextBlock
  else
    tip := nil;
...
begin
{$IFDEF FIB}
  result := GetsizeIndex_HeadIncluded(size);
{$ELSE}
...
begin
{$IFDEF FIB}
  h := GetsizeIndex_Headless(Size);
{$ELSE}
  h := highorderbit(Size)+1;
{$ENDIF}
...
begin
  ...
  size := pint(pp)^;
{$IFDEF FIB}
  h := GetSizeIndex_HeadIncluded(size);
{$ELSE}
  h := highorderbit(size)+1;
{$ENDIF}
  LockHeap(h);
...
begin
  pp := pointer(PAnsiChar(p)-LITE_HEADER_SIZE);
  oldsize := pint(pp)^;
  h := GetSizeIndexSizeFromBlockSizeInclHeader(oldsize);
