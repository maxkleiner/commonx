...
DiskInf GetDiskSta()
{
  ...
  char p [3];
  bool b_flag;
  DiskInf tmp;//The hard disk information structure defined in ICE SLICE tmp.TotalSpace = 0; tmp.FreeSpace = 0;//Get a valid drive name, which is the drive letter for (int drive = 1; drive <= 26; drive ++) {if (! _Chdrive (drive)) {memset (p, 0, sizeof (p)); p [0] = drive + 'A'-1; p [1] = ':'; p [2] = '\0'; b_flag = GetDiskFreeSpaceEx (p, & FreeAvailable, & TotalNum, & TotalFreeNum); if (b_flag) {tmp.TotalSpace + = (int) (TotalNum.QuadPart/(1024 * 1024) ); tmp.FreeSpace + = (int) (FreeAvailable.QuadPart/(1024 * 1024));}}
} return tmp;
}
...
uses windows, systemx;

const
  SYSTEM_BASIC_INFORMATION = 0;
  SYSTEM_PERFORMANCE_INFORMATION = 2;
  SYSTEM_TIME_INFORMATION = 3;
  SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION = 8;//<<--individual processors
...
type
  ...
  TSystem_Processor_Performance_Information = packed record
  ...
  TCPUUSage = record
    ...
    liSystemTime: LARGE_INTEGER;
    liActiveTime: LARGE_INTEGER;
...
function GetCPUUsage(previous: TCPUUsage): TCPUUsage;
function GetCPUUsages(aprevious: TArray<TCPUUsage>): TArray<TCpuUsage>;
...
begin
  ...
  var cpus:= SystemX.GetNumberOfLogicalProcessors;
  result.max := cpus;
...
//  begin
    ...
    // get new system time
    status := NtQuerySystemInformation(SYSTEM_TIME_INFORMATION, @SysTimeInfo, SizeOf(SysTimeInfo), 0);
    if status <> 0 then Exit;

    // get new CPU's idle time
    status := NtQuerySystemInformation(SYSTEM_PERFORMANCE_INFORMATION, @SysPerfInfo, SizeOf(SysPerfInfo), nil);
    if status <> 0 then Exit;

    // if it's a first call - skip it
    if (previous.liIdleTime.QuadPart <> 0) then
    begin

      // CurrentValue = NewValue - OldValue
      dbIdleTime := Li2Double(SysPerfInfo.liIdleTime) - Li2Double(previous.liIdleTime);
      dbSystemTime := Li2Double(SysTimeInfo.liKeSystemTime) - Li2Double(previous.liSystemTime);
      result.deltaIdleTime := Li2Double(SysPerfInfo.liIdleTime) - Li2Double(previous.liIdleTime);
      result.deltaSystemTime := Li2Double(SysTimeInfo.liKeSystemTime) - Li2Double(previous.liSystemTime);

      // CurrentCpuIdle = IdleTime / SystemTime
      dbIdleTime := dbIdleTime / dbSystemTime;

      // CurrentCpuUsage% = 100 - (CurrentCpuIdle * 100) / NumberOfProcessors
      var cpuTime := double(cpus) - dbIdleTime;
      if cputime < 0 then
        cputime := 0;
      result.usage := cputime;


      // Show Percentage
//      con.writeln(FormatFloat('CPU Usage: 0.0 %',cpuTime*100.0)+' / '+inttostr(cpus*100)+'%') ;

    end;

    result.liIdleTime := SysPerfInfo.liIdleTime;
    result.liSystemTime := SysTimeinfo.liKeSystemTime;
...
var
  SysBaseInfo: TSystem_Basic_Information;
  ASysPerfInfo: TArray<TSystem_Processor_Performance_Information>;
  SysPerfInfo : TSystem_Processor_Performance_Information;
...
begin
  ...
  var cpus:= SystemX.GetNumberOfLogicalProcessors;
  if length(aprevious) <> cpus then
    setlength(aprevious, cpus);
  setlength(aSysPerfInfo, cpus);


  setlength(result,cpus);
  //result.max := cpus;
...
//    if status <> 0 then Exit;
    ...
    // get new CPU's idle time
    status := NtQuerySystemInformation(SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION, @ASysPerfInfo[0], SizeOf(SysPerfInfo)*cpus, nil);
    if status <> 0 then Exit;

    for var t := 0 to cpus-1 do begin
      previous := aprevious[t];
      SysPerfInfo := aSysPerfInfo[t];
      var AbsActive := //SysPerfInfo.KernelTime.quadpart+
                       SysPerfInfo.dpcTime.quadpart+
                       SysPerfInfo.InterruptTime.quadpart+
                       SysPerfInfo.usertime.quadpart;
      var AbsIdle := SysPerfInfo.IdleTime.quadpart;
      var AbsTotal := AbsActive+AbsIdle;
      result[t].liIdleTime.QuadPart  := AbsIdle;
      result[t].liActiveTime.QuadPart := AbsActive;
      // if it's a first call - skip it
      if (previous.liIdleTime.QuadPart <> 0) then
      begin
        ...
        var DeltaActive := AbsActive - previous.liActiveTime.QuadPart;
        var DeltaIdle := AbsIdle - previous.liIdleTime.quadpart;
        var DeltaTotal := DeltaIdle+DeltaActive;
        result[t].deltaIdleTime := DeltaIdle;
        result[t].deltaSystemTime := Deltatotal;
        ...
        // CurrentCpuUsage% = 100 - (CurrentCpuIdle * 100) / NumberOfProcessors
        var cpuTime := DeltaActive/(DeltaTotal);
        if cputime < 0 then
          cputime := 0;
        result[t].usage := cputime;
        result[t].max := 1.0;
...
//      result[t].liSystemTime.quadpart := (SysPerfInfo.IdleTime.quadpart)+
//                                         (SysPerfInfo.KernelTime.quadpart)+
//                                          (SysPerfInfo.UserTime.quadpart);
