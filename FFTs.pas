...
uses
  FFTComplexs, FFTTypes, SysUtils, systemx;

const

  cMaxPrimeFactor     = 7919;//1021;
...
procedure ForwardFFT(var plan: TFFTPlan; const Source: TArray<TComplex>; var Dest: TArray<TComplex>; Count: integer);
procedure ForwardFFTEx(const Source,dest,scratch: PFFTComplexArray; Count: integer);
...
procedure InverseFFT(var plan: TFFTPlan; const Source: Tarray<TComplex>; var Dest: Tarray<TComplex>; Count: integer);
procedure InverseFFTEx(const Source,dest,scratch: PFFTComplexArray; Count: integer);

function FFTPower(var plan: TFFTPlan; var lDataIn,lDataOut: SingleP; lnIn: Integer): boolean;
...
begin
   ...
   SetLength(Dest, lnIn);
     ...
     ForwardFFT(plan, Source, Dest, lnIn);
...
procedure ReorderSeries(Count: integer; var Factors: TIdx1FactorArray; var Remain: TIdx0FactorArray;
  const X: Tarray<TComplex>; var Y: TArray<TComplex>);
var
  i, j, k: integer;
...
procedure ReorderSeriesEx(Count: integer; var Factors: TIdx1FactorArray; var Remain: TIdx0FactorArray;
  const X: PFFTComplexArray; Y: PFFTComplexArray);
var
  i, j, k: integer;
  Counts: TIdx1FactorArray;
begin
...
procedure FFT_2(var Z: TArray<TComplex>);
var
  T1: TComplex;
begin
...
procedure FFT_3(var Z: Tarray<TComplex>);
var
...
procedure FFT_4(var Z: Tarray<TComplex>);
var
...
procedure FFT_5(var Z: Tarray<TComplex>);
var
...
procedure FFT_8(var Z: TArray<TComplex>);
var
  A, B: Tarray<TComplex>;
  Gem: TFloat;
begin
...
procedure FFT_10(var Z: TArray<TComplex>);
var
  A, B: Tarray<TComplex>;
begin
  setlength(a,4);
...
procedure SynthesizeFFT(Sofar, Radix, Remain: integer; var Y: TArray<TComplex>);
var
  GroupOffset, DataOffset, Position: integer;
  GroupNo, DataNo, BlockNo, SynthNo: integer;
  Omega: double;
  S, CosSin: TComplex;
  Synth, Trig, Z: Tarray<TComplex>;
...
// main
begin
  setlength(trig, cMaxPrimeFactor);
  setlength(z, cMaxPrimeFactor);
  setlength(synth, cMaxPrimeFactor);
  // Initialize trigonomial coefficients
...
procedure SynthesizeFFTEx(Sofar, Radix, Remain: integer; Y: PFFTComplexArray);
var
  GroupOffset, DataOffset, Position: integer;
  GroupNo, DataNo, BlockNo, SynthNo: integer;
  Omega: double;
...
var
  ...
  ActualRadix: TIdx1FactorArray;
  RemainRadix: TIdx0FactorArray;
  TmpDest: PFFTComplexArray;
begin
  if Count = 0 then exit;
  if count > 10000 then
    raise Exception.create('wild count '+count.tostring);
  ...
  if (ActualRadix[1] > cMaxPrimeFactor) then
    raise EMathError.Create(sErrPrimeTooLarge+'  actual '+actualradix[1].tostring+'> primefactor:'+cMaxPrimeFactor.tostring);
  ...
  // synthesis
  ReorderSeriesEx(Count{, FactorCount}, ActualRadix, RemainRadix, Source, TmpDest);

  // Free the temporary copy (if any)
  if @Dest = @Source then begin
    MoveMem32(Dest, TmpDest, SizeOf(TComplex) * Count);
//    Move(TmpDest^, Dest, SizeOf(TComplex) * Count);
  ...
  for i := 1 to FactorCount do
    SynthesizeFFTEx(SofarRadix[i], ActualRadix[i], RemainRadix[i], Dest);
...
begin
  //plan.planned := false;
  if not plan.planned then begin
  if Count = 0 then exit;

  // Decompose the series with length Count into FactorCount factors in ActualRadix
  Factorize(Count, plan.FactorCount, plan.ActualRadix);
  plan.planned := true;
  end;


  // Check if our biggest prime factor is not too large
  if (plan.ActualRadix[1] > cMaxPrimeFactor) then
    raise EMathError.Create(sErrPrimeTooLarge);

  // Setup Sofar and Remain tables
  plan.RemainRadix[0] := Count;
  plan.SofarRadix[1]  := 1;
  plan.RemainRadix[1] := Count div plan.ActualRadix[1];
  for i := 2 to plan.FactorCount do begin
    plan.SofarRadix[i]  := plan.SofarRadix[i-1] * plan.ActualRadix[i-1];
    plan.RemainRadix[i] := plan.RemainRadix[i-1] div plan.ActualRadix[i];
  end;
  ...
  if @Dest = @Source then begin
    ...
    // Reorder the series so that the elements are already in the right place for
    // synthesis
    ReorderSeries(Count{, FactorCount}, plan.ActualRadix, plan.RemainRadix, Source, TmpDest^);
  ...
  // Synthesize each of the FFT factored series
  for i := 1 to plan.FactorCount do
    SynthesizeFFT(plan.SofarRadix[i], plan.ActualRadix[i], plan.RemainRadix[i], Dest);
...
begin
  ...
  ForwardFFT(plan, TmpSource, Dest, Count);
...
begin
  ...
  for i := 0 to Count - 1 do
    with Source[i] do
      TmpSource[i] := Complex(Re, -Im);
  ForwardFFTEx(@TmpSource, Dest, scratch, Count);
