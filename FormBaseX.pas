...
uses
  Windows, anoncommand, vcl.themes, vcl.styles, fatmessage,
...
type
  ...
  published
    ...
    procedure WaitForSinglecommand(c: TCommand; timeout: ticker = 0);
...
procedure TfrmBase.WaitForSinglecommand(c: TCommand; timeout: ticker = 0);
var
  wasenabled: boolean;
  tmLastUpdate: ticker;
begin
...
{$IFDEF LOCALCOMMANDWAIT}
  if timeout > 0 then
    raise ENotImplemented.Create('timout>0 is not implemented yet');
  ...
  try
    ...
    try
      ...
      while not c.IsComplete do begin
        var slptm := 1000 div 120;
        if timeout > 0 then slptm := greaterof(timeout, slptm);
        sleep(slptm);
...
{$ELSE}
  ...
  try
    progressform.frmProgress.WatchSingleCommand(c, timeout);
  finally
...
begin
  ...
  while (FMonitoringCommands.count > 0) and (FMonitoringCommands[0].IsComplete) do begin
    try
      FMonitoringCommands[0].RaiseExceptions := false;
      FMonitoringCommands[0].WaitFor;
    finally
      var c := FMonitoringCommands[0];
