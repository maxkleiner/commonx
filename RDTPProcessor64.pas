...
uses
  helpers_winsock, dtnetconst,
  numbers, simpleabstractconnection, packet, systemx, typex, sysutils, windows, classes, betterobject, sharedobject, SQLExpr, stringx, AbstractRDTPDataModule,  simplewinsock, exceptions, commandprocessor,orderlyinit, helpers.list, storageenginetypes;
...
type
  ...
  public
    ...
    procedure DefineFunc(sService: string; iRequest: int64;  func: TRDTPFunction);
    function GetFunction(sServiceName: string; iRequest: int64): TRDTPFunction;
  end;
  ...
  private
    ...
    function GetCommandQueue(idx: int64): TRDTPPacket;
    function GetCommandsInQueue: int64;
  ...
  protected
    ...
    function callback(var Packet: TRDTPPacket; iTimeOutMS: int64 = 300000; slDebug: TStringList = nil; bForget: boolean = false): boolean;
  ...
  public
    ...
    procedure SendProgress(sMessage: string; iPos, iMax: int64; sDebugLog: string = '');
    procedure OnConnectionProgress(sMessage, sDebug: string; iPos, iMax: int64);
    property CallbackTimeout: int64 read FCallbackTimeout write FCallbackTimeout;

    property CommandQueue[idx: int64]: TRDTPPacket read GetCommandQueue;
    property CommandsInQueue: int64 read GetCommandsInQueue;
...
procedure TRDTPFunctions.DefineFunc(sService: string; iRequest: int64;
  func: TRDTPFunction);
var
...
function TRDTPProcessor.GetCommandQueue(idx: int64): TRDTPPacket;
begin
  result := self.FCommandQueue[idx];
end;

function TRDTPProcessor.GetCommandsInQueue: int64;
begin
...
{$IFDEF DEBUG_RDTP}        Debug.Log('Read packet from Queue: '+memorytohex(result.DecryptedBuffer.FRealBuffer, result.DecryptedBuffer.Length), 'RDTPPLUMBING');{$ENDIF}
    ...
    //read 26 bytes
    iRead := 0;
    while iRead < PACKET_HEADER_SIZE do begin
      iToRead := PACKET_HEADER_SIZE-iRead;
      ...
      iJustRead := FSocket.GuaranteeReadData(@(pbyte(@header)[iRead]), iToRead,8000, true);
      if iJustRead = 0 then
        exit(nil);
...
{$IFDEF DEBUG_RDTP}
    Debug.Log('Got a header, it is:'+memorytohex(@header, PACKET_HEADER_SIZE),'RDTPPLUMBING');
{$ENDIF}
    len := header.Packedlength+sizeof(header);
    if len < dtnetconst.PACKET_HEADER_SIZE then begin
      FSocket.Disconnect;
      raise ETransportError.create('packet was not at least '+inttostr(PACKET_HEADER_SIZE)+' bytes! ('+inttostr(len)+')');
    end;
{$IFNDEF ENABLE_HUGE_MESSAGES}
    if len > 16*BILLION then begin

      raise ETransportError.create('packet length is unusually large! '+commaize(len)+' bytes '+len.tohexstring);
    end;
{$ENDIF}
    PC := GetMemory(len);
    MoveMem32(@pc[0], @header, PACKET_HEADER_SIZE);

    iRead := PACKET_HEADER_SIZE;
{$IFDEF DEBUG_RDTP}    Debug.Log('Decoded unpacked header length:'+inttostr(header.unpackedlength),'RDTPPLUMBING');{$ENDIF}
{$IFDEF DEBUG_RDTP}    Debug.Log('Decoded packed header length:'+inttostr(header.packedlength),'RDTPPLUMBING');{$ENDIF}
...
{$IFDEF DEBUG_RDTP_ex}    Debug.Log('read complete, assigning buffer to packet: '+memorytohex(pc, header.length),'RDTPPLUMBING');{$ENDIF}
    result := TRDTPPAcket.create;
    result.EncryptedBuffer.AssignBuffer(pc, header.Packedlength);

{$IFDEF DEBUG_RDTP}    Debug.Log(self,'Read a packet that is '+inttostr(header.packedlength)+' (packed) bytes','RDTPPLUMBING');{$ENDIF}
{$IFDEF DEBUG_RDTP}    Debug.Log(self,'Read a packet that is '+inttostr(header.unpackedlength)+' (unpacked) bytes','RDTPPLUMBING');{$ENDIF}
{$IFDEF DEBUG_RDTP}    Debug.Log(self,MemoryDebugString(pc, PACKET_HEADER_SIZE),'RDTPPLUMBING');{$ENDIF}
//{$IFDEF DEBUG_RDTP}    Debug.Log(self,MemoryDebugString(pc+PACKET_HEADER_SIZE, header.length-PACKET_HEADER_SIZE),'RDTPPLUMBING');{$ENDIF}
...
xxx
  i := Fsocket.PeekBuf(iBuf, sizeof(int64));
...
procedure TRDTPProcessor.SendProgress(sMessage: string; iPos, iMax: int64;
  sDebugLog: string);
var
  prog: TRDTPPacket;
  tmNow: cardinal;
...
function TRDTPProcessor.callback(var Packet: TRDTPPacket; iTimeOutMS: int64;
...
{$IFDEF DEBUG_RDTP_PLUMBING}  Debug.Log(self,'About to WRITE a packet that is '+inttostr(packet.UnPackedLength)+' bytes','RDTPPLUMBING');{$ENDIF}
  if packet.UnPackedLength > 1000000 then
    Debug.Log('large');
  pc := packet.decryptedbuffer.FRealBuffer;
{$IFDEF DEBUG_RDTP_PLUMBING}   Debug.Log(self,MemoryToHex(pc, PACKET_HEADER_SIZE), 'RDTPPLUMBING');{$ENDIF}
//{$IFDEF DEBUG_RDTP_PLUMBING}   Debug.Log(self,MemoryToHex(pc+PACKET_HEADER_SIZE, packet.packedlength-PACKET_HEADER_SIZE), 'RDTPPLUMBING');{$ENDIF}

  if packet.IsCRCValid then begin
    if packet.IsMarkerValid then begin
{$IFDEF RDTP_LOGGING_EX}      Debug.Log(self,packet.GetDebugMessage,'RDTPPLUMBING');{$ENDIF}
    end else
      Debug.Log(self,'MArker is **INVALID**','RDTPPLUMBING');
  end else begin
...
function TRDTPFunctions.GetFunction(sServiceName: string; iRequest: int64): TRDTPFunction;
var
