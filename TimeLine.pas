...
const
  ...
  POINT_OUTER_X_RADIUS = 3;
  POINT_INNER_X_RADIUS = 2;
  POINT_GLOW_SCALE = 10;
  POINT_GLOW_YSCALE = 3;
...
type
  ...
  TTimelineCommand = (tlcStartloop, tlcStopLoop, tlcBankSetChange, tlcOther);
  ...
  public
    ...
    procedure Conclude;
    procedure AddPoint(rTime: nativefloat; iLine: integer; sCommand: string);
    ...
    procedure CreateHeader(stagefilename: string);
  end;
...
procedure TTimeLine.AddPoint(rTime: nativefloat; iLine: integer; sCommand: string);
var
  p: tTimeLInePoint;
  tll: TTimelineline;
begin
...
var
  ystart, yFinish, xstart, xend: integer;
  t, u: integer;
begin
  ystart := mouse_down_at.Y;
  yFinish := mouse_last_pos_for_wheel.Y;
  ...
  for u := 0 to FLInes.Count - 1 do begin
    ...
    xstart := mouse_down_at.X;
    xend := mouse_last_pos_for_wheel.X;
    order(xstart, xend);
    FLInes[u].SelectPointsInRange(ScreentoGlobalx(xstart),
      ScreentoGlobalx(xend), true);
  end;
...
begin
  ...
  for u := 0 to fLInes.count-1 do begin
    ...
    if ltlp.BoolVal = tbTrue then begin
      AddPoint(tlp.Time+1, FLines.IndexOf(tll), 'StopLoop');
...
procedure TTimeLine.CreateHeader(stagefilename: string);
begin
  while slHeader.Count < 3 do begin
    slHeader.Add('');
  end;
  slHeader[2] := ExtractFileName(stagefilename);
end;
...
begin
  inherited;
  tll := GetTimeLineLineAtScreenPoint(mouse_last_pos_for_wheel.x,mouse_last_pos_for_wheel.y);

  if tll = nil then exit;

  AddPoint(ScreenToGlobalX(mouse_last_pos_for_wheel.x), tll.tag,  'StartLoop');

//  p := tll.AddPoint;
//  p.FCommand := 'StartLoop';
//
//  p.Time := ScreenToGlobalX(mouse_last_pos_for_wheel.x);
//
//  p.BoolVal := tbTrue;
//  p.Param1 := tll.Tag div 5;
//  p.Param2 := tll.Tag mod 5;
...
//
//
//  CreateExplosion(mouse_last_pos_for_wheel.x, mouse_last_pos_for_wheel.y, tll.color);
  ...
  for u := 0 to FLines.count-1 do begin
    ...
    for t:= tll.Fpoints.count-1 downto 0 do begin
      ...
      if p.selected then begin
        ...
        if xx > self.clientwidth then continue;
        if yy > self.clientwidth then continue;
...
// draw the scrubber
  ...
  if mouse_op_mode = momDragSelect then begin
    ...
    self.canvas_Rectangle_Empty(mouse_down_at.X, mouse_down_at.Y,
      mouse_last_pos_for_wheel.X, mouse_last_pos_for_wheel.Y, clAqua, clAqua, clAqua,
      clAqua, 1.0);
    self.canvas_Rectangle_Fill(mouse_down_at.X, mouse_down_at.Y,
      mouse_last_pos_for_wheel.X, mouse_last_pos_for_wheel.Y, clAqua, clAqua, clAqua, clAqua, 0.2);
    Alphaop := aoNone;
  end;
...
begin
  ...
  cPercent := (ScreenToGlobalX(mouse_last_pos_for_wheel.x) - TargetBoundX1) / w;
...
begin
  ...
  if (Button = mbRight) then begin
    ...
    if tlp = nil then begin
      self.Position := ScreenToGlobalX(mouse_last_pos_for_wheel.x);
...
begin
  result := mouse_last_pos_for_wheel.X - mouse_down_at.X;
end;

function TTimeLine.MouseMovedY: integer;
begin
  result := mouse_last_pos_for_wheel.Y - mouse_down_at.Y;
...
begin
  ...
  if mouse_op_mode = momMovePoint then begin
    MoveSelectedPoints(self.ScaleScreenXtoGlobal(mouse_last_pos_for_wheel.x - mouse_down_at.X, true),true);
  end;
  if mouse_op_mode = momDragSelect then begin
...
begin
  ...
  if GetFirstPointInWindow(t) then begin
    while true do begin
      ...
      // start points
      if Fpoints[t].Command = 'StartLoop' then begin
        Parent.Rectangle(Fpoints[t].Time - Parent.ScaleScreenXtoGlobal
...
begin
  result := nil;
  t := 0;
  while t < fpoints.count do begin
    p := FPoints[t];
