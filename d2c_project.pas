...
uses
  typex, debug, SysUtils, Types, Classes, Graphics, Controls, Forms, Dialogs, windows, beeper, generics.collections.fixed,
  StdCtrls, advancedgraphics, messages, xmltools, math, namevaluepair, variants, sharedObject, betterobject;
const
  MK_LBUTTON = 1;
  MK_RBUTTON = 2;
  MK_SHIFT = 4;
...
type
  ...
  TSymbolScope = class(TBetterObject)
  private
    FParentScope: TSymbolScope;
    FSymbols: TSymbolList;
  public
    property Symbols: TSymbolList read FSymbols write FSymbols;
    property ParentScope: TSymbolScope read FParentScope write FParentScope;
    function ResolveSymbol(sName: string; sSignature: string = ''): TSymbol;
  end;

  TSymbolStack = class(TBetterObject)
  private
    FTop : TSymbolScope;
  public
    property Top: TSymbolScope read FTop;
    procedure PushNew;
    procedure PopFree;
  end;
  ...
  private
    ...
    procedure SetProject(const Value: TD2CProject);
  ...
  public
    ...
    constructor create(sName: ansistring; oProject: TD2CProject; parent: TSymbol);virtual;
    ...
    property Project: TD2CProject read FProject write SetProject;
    ...
    property Debug: ansistring read FDebug write FDebug;
...
//    FName: ansistring;
    ...
    FBuiltIn: boolean;
  public
    constructor create(sName: ansistring; oProject: TD2CProject; parent: TSymbol); override;
    destructor destroy; override;
//    property Name: ansistring read FName write FName;
  ...
  TPrimitiveSymbol = class(TSymbol)
  private
    FPrimitive: boolean;
    FCtype: string;
  public
    constructor create(sName: ansistring; oProject: TD2CProject; parent: TSymbol; bPrimitive: boolean = false; sCTYPE: string = ''); reintroduce;virtual;
    property Primitive: boolean read FPrimitive write FPrimitive;
  ...
  TSymbolInstance = class(TSymbol)
  private
    FResolvedType: TSymbol;
    FTypeName: string;
    FInstanceName: string;
  public
    property TypeName: string read FTypeName write FTypeName;
    property ResolvedType: TSymbol read FResolvedType write FResolvedType;
  end;

  TParameterDefinition = class(TSymbolInstance)
  private
  ...
  TParameterList = class(TList<TParameterDefinition>)
  public
  ...
  public
    constructor create(oProject: TD2CProject; diagram: TDiagram); reintroduce; virtual;
    destructor Destroy; override;
  ...
  public
    constructor create(oProject: TD2CProject); reintroduce; virtual;
    destructor Destroy; override;
    property Project: TD2CProject read FProject write FProject;
  ...
  public
    ...
    function AddItem(sName: ansistring): TcompartmentItem; overload;virtual;
...
(*  TStructure = class(TSquareSymbol)
  private
    ...
    function GetMembers(iIndex: integer): TMemberDefinition;
  public
    constructor create(oProject: TD2CProject; parent: TSymbol); override;
    destructor Destroy; override;
    procedure Paintmain(dd: TdiagramDisplay; mr: TDiagramMemberRecord);override;
    procedure DrawCompartmentItem(dd: TdiagramDisplay; mr: TDiagramMemberRecord; iPosition: integer; sSignature: ansistring);
    function IsRoomForCompartmentItem(dd: TdiagramDisplay; mr: TDiagramMemberRecord; iPosition: integer): boolean;
  ...
  //----------------------------------------------------------------------------
  TMatureConstruct = class(TRecord)
  private
    FunitName: ansistring;
    FAncestors: TSymbolList;
    ...
    function GetAncestor(iIndex: integer): TMatureConstruct;
  ...
  public
    ...
    property Ancestors[iIndex: integer]: TMatureConstruct read GetAncestor;
    procedure AddAncestor(sym: TMatureConstruct);
  ...
  private
    ...
    function GetProject: TD2CProject;
    procedure SetProject(const Value: TD2CProject);
  public
    constructor create(oProject: TD2CProject); reintroduce; virtual;
    destructor Destroy; override;
    ...
    property Count: integer read GetCount;
    property Project: TD2CProject read GetProject write SetProject;
  ...
  //----------------------------------------------------------------------------
  TD2CProject = class(TLockQueuedObject)
  private
    FSymbols: TSymbolList;
  ...
  public
    ...
    procedure Clear;
    procedure DefineBuiltInSymbols;
    procedure DefineBuiltInPrimitiveSymbol(sPascalType, sCType: string);
...
function ProjectToXML(project: TD2CProject): ansistring;
...
uses Dir, d2c_pascalcompiler, stringx, DirFile, ccompiler;
...
constructor TDiagramList.create(oProject: TD2CProject);
begin
  inherited Create;
...
//------------------------------------------------------------------------------
function TDiagramList.GetProject: TD2CProject;
begin
  LockRead;
  try
    result := FProject;
...
constructor TDiagram.create(oProject: TD2CProject);
begin
  FName := 'new diagram';
  FProject := oProject;
...
//------------------------------------------------------------------------------
constructor TDiagramMemberRecord.create(oProject: TD2CProject; diagram: TDiagram);
begin
  inherited create;
  FSelected := false;
  FProject := oProject;
...
constructor TSymbolList.create(oProject: TD2CProject);
begin
  inherited Create;
  FProject := oProject;
...
//------------------------------------------------------------------------------
function TSymbolList.GetProject: TD2CProject;
begin
  LockRead;
  try
    result := FProject;
...
{ TSquareSymbol }
//------------------------------------------------------------------------------
constructor TSquareSymbol.create(sNAme: ansistring; oProject: TD2CProject; parent: TSymbol);
begin
...
{ TProject }
//------------------------------------------------------------------------------
procedure TD2CProject.AddSourceDir(sPath, sFilter: ansistring; bRecurse: boolean = true);
var
  dir: TDirectory;
  t: integer;
begin
...
procedure TD2CProject.AddSourceFile(sFullName: ansistring);
begin
  LockString;
  try
...
procedure TD2CProject.Clear;
begin
  Symbols.clear;
end;

procedure TD2CProject.Compile(sFileName: ansistring);
var
  compiler: TCompiler;
  sym: Tsymbol;
  s: ansistring;
...
procedure TD2CProject.CompileAll;
var
  sFile: ansistring;
  t: integer;
begin
...
procedure TD2CProject.CompileQueue;
var
...
constructor TD2CProject.create;
begin
  inherited;
...
procedure TD2CProject.DefineBuiltInPrimitiveSymbol(sPascalType, sCType: string);
var
  sym: TPrimitiveSymbol;
begin
  sym := TPrimitiveSymbol.create(sPascalType, self, nil, true, sCType);
end;

procedure TD2CProject.DefineBuiltInSymbols;
var
  sym: TSymbol;
begin
  sym := TSymbol.create('writeln', self, nil);

end;

destructor TD2CProject.Destroy;
begin
  FSymbols.free;
  Fdiagrams.free;
  FSourceFiles.Free;
...
//------------------------------------------------------------------------------
function TD2CProject.GetDiagrams: TDiagramList;
begin
  result := Fdiagrams;
end;
//------------------------------------------------------------------------------
function TD2CProject.GetFileName: ansistring;
begin
  result := FFileName;
end;
//------------------------------------------------------------------------------
function TD2CProject.GetSourceFileCount: integer;
begin
  LockRead;
  try
...
function TD2CProject.GetSourceFiles(idx: integer): ansistring;
begin
  LockString;
  try
...
function TD2CProject.GetSymbols: TSymbolList;
begin
  result := FSymbols;
end;
//------------------------------------------------------------------------------
function TD2CProject.GetTitle: ansistring;
begin
  LockString;
  try
    result := FTitle;
...
procedure TD2CProject.QueueCompile(sFileNAme: ansistring);
begin
...
procedure TD2CProject.Save;
begin
  raise Exception.create('Not Implemented');
end;
//------------------------------------------------------------------------------
procedure TD2CProject.SetFileName(const Value: ansistring);
begin
  FFileName := Value;
end;
//------------------------------------------------------------------------------
...
constructor TSymbol.create(sName: ansistring; oProject: TD2CProject; parent: TSymbol);
begin
  inherited create;
  FAuthor := '';
  FProject := oProject;
...
function TRecord.AddItem(sName: ansistring): TCompartmentItem;
var
  ci: TCompartmentItem;
begin
...
procedure TRecord.AddItem(item: TcompartmentItem);
begin
  if item = nil then
...
function TRecord.AddMember(sName: ansistring): TMemberDefinition;
begin
...
constructor TRecord.create(sName: ansistring; oProject: TD2CProject;
  parent: TSymbol);
begin
  inherited;
  FItems := TList.create;
...
//------------------------------------------------------------------------------
procedure TRecord.DeleteItem(iIndex: integer);
var
  ci: TCompartmentItem;
begin
  ci := Items[iIndex];
...
//------------------------------------------------------------------------------
procedure TRecord.DrawCompartmentItem(dd: TdiagramDisplay;
...
//------------------------------------------------------------------------------
function TRecord.GetItemCount: integer;
begin
  result := FItems.Count;
end;

function TRecord.GetItems(iIndex: integer): TCompartmentItem;
begin
  result := TCompartmentItem(FItems[iIndex]);
end;

function TRecord.GetItemTypeCount(cClass: TCompartmentClass): integer;
...
//------------------------------------------------------------------------------
function TRecord.GetMemberCount: integer;
begin
  result := self.GetItemTypeCount(TMemberDefinition)
end;

function TRecord.GetMembers(iIndex: integer): TMemberDefinition;
begin
  result := GetNthClassInstance(iIndex, TMemberDefinition) as TMemberDefinition;

end;
//------------------------------------------------------------------------------
function TRecord.GetNthClassInstance(n: integer;
...
function TRecord.IsRoomForCompartmentItem(dd: TdiagramDisplay;
...
//------------------------------------------------------------------------------
procedure TRecord.PaintMain(dd: TdiagramDisplay;
  mr: TDiagramMemberRecord);
var
  r,g,b: integer;
  x1,y1,x2,y2: integer;
...
//------------------------------------------------------------------------------
constructor TMatureConstruct.create(sName: ansistring; oProject: TD2CProject;
  parent: TSymbol);
begin
  inherited;
  FAncestors := TSymbolList.create(self.Project);
...
//------------------------------------------------------------------------------
constructor TCompartmentItem.Create(sName: ansistring; oProject: TD2CProject; parent: TSymbol);
begin
  inherited;
  FAlias := '';
  Fauthor := '';
...
//------------------------------------------------------------------------------
function TMatureConstruct.GetAncestor(iIndex: integer): TMatureConstruct;
begin
  result := FAncestors[iIndex] as TMatureConstruct;
end;
...
//------------------------------------------------------------------------------
constructor TAttachedLineSymbol.create(sNAme: ansistring; oProject: TD2CProject; parent: TSymbol);
begin
  inherited;
  FSourceSymbol := nil;
  FTargetSymbol := nil;
...
procedure TSymbol.SetProject(const Value: TD2CProject);
begin
...
procedure TSymbolList.SetProject(const Value: TD2CProject);
begin
  LockWrite;
  try
    FProject := value;
...
procedure TD2CProject.SetTitle(const Value: ansistring);
begin
  LockString;
  try
    FTitle := Value;
...
procedure TDiagramList.SetProject(const Value: TD2CProject);
begin
  LockWrite;
  try
    FProject := value;
...
function TD2CProject.SymbolToUnitName(sSym: ansistring): ansistring;
var
...
constructor TPrimitiveSymbol.create(sName: ansistring; oProject: TD2CProject;
  parent: TSymbol; bPrimitive: boolean; sCTYPE: string);
begin
  inherited create(sName, oProject, parent);
...
procedure TParameterList.DestroyChildren;
var
  i: TParameterDefinition;
begin
...
function TSymbolScope.ResolveSymbol(sName, sSignature: string): TSymbol;
begin
  result := SYmbols.FindSymbol(sName);

  if result = nil then
    result := ParentScope.ResolveSymbol(sName, sSignature);

end;

{ TSymbolStack }

procedure TSymbolStack.PopFree;
begin
...
procedure TSymbolStack.PushNew;
begin
