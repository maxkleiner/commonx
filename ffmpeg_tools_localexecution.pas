unit ffmpeg_tools_localexecution;
{$DEFINE USE_MFS}
{$DEFINE DO_THUMB}
{$DEFINE DO_OQ}
{$DEFINE DO_UQ}
...
uses
  ...
  soundconversions_commandline, rtti_helpers, commands_file, orderlyinit, zip,
...
function ffMpegProbeStr(sFile: string; bSynchronous: boolean = false): string;
...
procedure VideoToDmxVideo(bKaraoke: boolean; sOriginalInputFile: string; sOutputFile: string; cmd: TCommand = nil);
...
var
  ProbeCommands: TCommandProcessor;

implementation

uses
  servervideoparser, tools, audai;
...
//{$IFNDEF REXE}
    try
      if extractfilepath(outputfile) <> '' then begin
        forcedirectories(extractfilepath(outputfile));
      end;
    except
    end;
...
begin
  ...
  try
    ...
    ffmpeg_tools_localexecution.ExtractVideoFrames(sIn, sTemp);
  //  ffmpeg_tools.ConformVideoFiles(sTemp, true, true);
    ...
    case fmt of
      jvfJPG: ffmpeg_tools_localexecution.CReateJVidFromFrames_JPG(iLen, sTemp, sOut, '*.jpg', cmd);
      jvfCRN: ffmpeg_tools_localexecution.CReateJVidFromFrames_Crunch(iLen, sTemp, sOut, '*.jpg', fmt, 128, cmd);
      jvfDDS: ffmpeg_tools_localexecution.CReateJVidFromFrames_Crunch(iLen, sTemp, sOut, '*.jpg', fmt, 255, cmd);
    end;
...
begin
  ...
  sInputFile := sOriginalInputFile;
  if comparetext(extractfileext(sOriginalInputFile),'.zip')=0 then begin
    sInputFile := '';
    var unzippedTo := shellunziptotempfolder(sOriginalInputFile);
    var dir := TDirectory.CreateH(unzippedto,'*.*',0,0);
    while dir.o.GetNextFile(fil) do begin
      var tofil := extractfilepath(soutputfile)+changefileext(fil.name,'.zip'+fil.extension);
      copyfile(fil.FullName,tofil);
      if comparetext(fil.extension,'.mp4')=0 then begin
        sInputFile := fil.fullname;
      end;
    end;
    if sInputFile = '' then
      exit;
  end;
  var sMainExt := lowercase(extractfileext(sInputFile));
  var bMP3Input := (sMainExt = '.mp3');
  var bAudioOnly := bMp3Input or (sMainExt = '.wmv') or (sMainExt = '.ogg') or (sMainExt = '.ac3') or (sMainExt = '.wav');
  ...
  try
    try
      ...
      sCDGFile := sOutputFile+'.cdg';
      bNeedsMp3 := not fileexists(sFinalMp3);
      if bMp3input and bNeedsMp3 then begin
        copyfile(sInputFile, sFinalMp3);
        bNeedsMp3 := false;
      end;

      bNeedsMp4 := (not fileexists(sFinalMp4)) and (not bAudioOnly);
      bNeedsTurb := (not fileexists(sFinalTurb)) and (not bAudioOnly);
      bNeedsHue := (not fileexists(sFinalHue)) and (not bAudioOnly);
      ...
      sTemp2 := GetTempPath+'videoproc\'+inttostr(GetCurrentThreadID())+'\';
      forcedirectories(sTemp2);
      var sTempVid := sTemp2+extractfilename(sIn);
      copyfile(pchar(sIn), pchar(sTempVid), false);
      if not fileexists(sTempVid) then
        raise ECritical.create('could not copy '+sIn+' to '+sTempVid);
      ...
      try
        ...
        if bNeedsMp4 then begin
          ...
          c2.InputFile := sIn;
          c2.outputFile := sFinalmp4;
//          c2.suite := self;
          c2.Start;
{$IFDEF SYNC_DEBUG}
          c2.waitfor;
          if not fileexists(c2.outputfile) then
            raise ECritical.create('poop');
{$ENDIF}
        ...
        //  ffmpeg_tools.ConformVideoFiles(sTemp, true, true);
          ...
          if not fileexists(sFinalMp3) then
            raise ECritical.create('poop, no mp3');
...
{$IFDEF SYNC_DEBUG}
            turb.waitfor;
            if not fileexists(sFinalTurb) then
              raise ECritical.create('poop');
{$ENDIF}
...
{$IFDEF SYNC_DEBUG}
                  c3.waitfor;
                  if not fileexists(sFinalHue) then
                    raise ECritical.create('poop');
{$ENDIF}
...
begin
  ...
  VideoToDmxVideo(false, InputFile, OutputFile, self);
...
var
  s: string;
begin
  if comparetext('.mp3',extractfileext(sFile))=0 then begin
    result.init;
    result.AudioCodec := 'mp3';
    result.AudioCodecEx := 'mp3';
...
procedure oinit;
begin
  ProbeCommands := TCommandProcessor.create(nil,'ProbeCommands');
end;

procedure ofinal;
begin
  ProbeCommands.WaitForAll;
  ProbeCommands.free;
  ProbeCommands := nil;
...
init.registerprocs('ffmpeg_tools_localexecution',oinit,ofinal,'CommandProcessor');
