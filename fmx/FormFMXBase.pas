...
type
  ...
  public
    IsHomeForm: boolean;
    glassban: boolean;//attempted hack to clear up AVs when closing sometimes
    ...
    procedure DealWithOnscreenKeyboard(bvisible: boolean; keyboardrect: TRectF);virtual;
    ...
    procedure Curtains(proc: TProc; postproc_usuallynil: TProc = nil);
...
procedure TfrmFMXBase.Curtains(proc: TProc; postproc_usuallynil: TProc = nil);
begin
  //defer to mock-mobile form if assigned, in this case, this form will not handle curtains
...
procedure TfrmFMXBase.DealWithOnscreenKeyboard(bvisible: boolean;
  keyboardrect: TRectF);
begin
  //
end;
...
{$IFnDEF MSWINDOWS}
  ...
  self.Left := round(r.Left);
  self.Top := round(r.Top);
  self.Width := round(r.width);
  self.Height := round(r.Height);
...
begin
  ...
  try
    ...
    var p := InlineProcWithGuiEx(proc, procedure begin guisuccess() end, procedure(s: string) begin WorkError(s) end);
...
begin
  ...
  try
    ...
    Debug.Log('hard work command added');
...
begin
  ...
  for var t := 0 to high(default_curtains) do begin
    ...
    if default_curtain = nil then begin
      ...
      default_curtain.Stroke.Kind := TBrushKind.None;
...
{$IFDEF MSWINDOWS}
  if not (csDesigning in ComponentState) then begin
    if value and ((self <> application.mainform) and (application.mainform <> nil))then begin
...
begin
//  if not WorkingHard then
//    ToggleBusy(false);

  var processed: ni := 0;
  var allowance := greaterof(12,lesserof(gettimesince(tmOfLastMessageProcess),4000));
  try
...
//  try
    ...
    while mq.ProcessNextMessage do begin
      inc(processed);
      if gettimesince(tmStart) > allowance then begin
        tmFormMessages.interval := 1;
        exit;
      end;
    end;
...
//      Debug.Log('processed '+processed.tostring+' messages')
    end else
      tmFormMessages.interval := 100;

    tmOfLastMessageProcess := getticker;
  end;
