...
{$ELSE}
    FWaitingOn: Tcommand;
    FBackgroundOp: TLocalBackground;
{$ENDIF}
...
{$IFDEF ALLOW_BACKGROUND_OP_DEPRECATED}
...
{$ENDIF}
    ...
    property UpdatingState: boolean read GetUpdatingState;
  end;
...
begin
  ...
  FBackgroundOp := TLocalBackground.Create(AthreadFunc,
    ...
    true, false);
  FBackgroundOp.RaiseExceptions := true;
  FBackGroundOp.Start;
...
{$ENDIF}
      try
        ...
        if FBackgroundOp = FWaitingon then begin
          if FWaitingOn.Error then
            FBAckgroundOp.OnErrorProc(Exception.create(FWaitingon.ErrorMessage));
          FBackgroundOp := nil;
        end;
...
begin
  ics(Frefsect, classname+'-refSect');
  inherited;
...
function TfrmFMXBaseAdvanced.GetUpdatingState: boolean;
begin
  result := FUpdatingState > 0;
end;
...
//        overlay.Left := self.left;
    ...
    if fancyAnim <> nil then begin
      fancyAnim.visible := false;
      fancyAnim.Parent := nil;
      fancyAnim.DisposeOf;
//      fancyAnim.free;
    end;
    fancyAnim := nil;
...
begin
  if BusyTimer.Enabled = working then
    exit;
