...
[ComponentPlatformsAttribute($FFFF)]
  ...
  private
    ...
    procedure HandleRealign(bSecondPass: boolean);
  protected
...
function UpgradeLayoutToProportional(lay: TControl): TScaledLayoutProportional;
...
procedure TScaledLayoutProportional.HandleRealign(bSecondPass:boolean);
begin
...
//        sy := sz;
        ...
        for var t := 0 to self.ChildrenCount-1 do begin
          if children[t] is TControl then begin
            Debug.Log('scale '+children[t].name+' to '+floatprecision(sx,2)+'x'+floatprecision(sy,2));
...
procedure Register;
begin
   RegisterComponents('DigitalTundra',[TScaledLayoutProportional]);
