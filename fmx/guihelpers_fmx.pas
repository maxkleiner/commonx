...
uses
  ...
  FMX.ListBox, fmx.layouts, types, stringx;
...
procedure timer_disableall(owner: TFMXObject);
function IndexOfTag(cb: TComboBox; tag: int64): nativeint;
function ComboBoxSelectByTag(cb: TComboBox; tag: int64): boolean;
function IndexOfListItem(lv: TListBox; itm: TListBoxItem): ni;
function ControlValueAsString(c: TFMXObject): string;
procedure SaveControlToNVP(nvpl: TNameValuePairList; c: TFMXObject);
procedure LoadControlFromNVP(nvpl: TNameValuePairList; c: TFMXObject);
...
type
  ...
  public
    ...
    class procedure ArrayToListView<T_must_cast: record>(a: TArray<T_must_cast>; lv: TListView; proc: TProc<T_must_cast,TListViewItem>);static;
  end;
...
class procedure TGuiHelper.ArrayToListView<T_must_cast>(a: TArray<T_must_cast>; lv: TListView;
  proc: TProc<T_must_cast,TListViewItem>);
begin
  lv.beginupdate;
  try
    var ii := lv.ItemIndex;
    lv.items.clear;
    while lv.items.count < length(a) do
      lv.items.add;
    while lv.items.count > length(a) do
      lv.items.delete(lv.items.count-1);

    for var t := 0 to high(a) do begin
      proc(a[t],lv.items[t] as TListViewItem);
    end;
    lv.itemindex := ii;
  finally
    lv.EndUpdate;
...
procedure timer_disableall(owner: TFMXObject);
begin
  for var t := 0 to owner.Componentcount-1 do begin
    if owner.Components[t] is TTimer then
      (owner.components[t] as TTimer).Enabled := false;
  end;

end;
function ComboBoxSelectByTag(cb: TComboBox; tag: int64): boolean;
begin
  var idx := indexOfTag(cb,tag);
  result := idx >=0;
  cb.ItemIndex := idx;
end;
...
begin
  result := -1;
  for var t := 0 to cb.Count-1 do begin
    if cb.listitems[t].tag = tag then
      exit(t);
  end;
end;
...
begin
  if c is TControl then begin
    var cc := c as Tcontrol;
    if cc is TEdit then begin
      exit((cc as TEdit).Text);
    end;
    if cc is TCheckbox then begin
      if (cc as TCheckbox).ischecked then
        exit('1')
      else
        exit('0');
    end;

    if cc is TRadioButton then begin
      if (cc as TRadioButton).ischecked then
        exit('1')
      else
        exit('0');
    end;

    if cc is TComboBox then begin
      if nil <> (cc as TComboBox).Selected then
        exit((cc as TComboBox).Selected.Text)
      else
        exit('');

    end;

    if cc is TListBox then begin
      var lb := cc as TListBox;
      result := '';
      for var t:= 0 to lb.Count-1 do begin
        if lb.ListItems[t].IsSelected then
          result := AOR(result,',',inttostr(lb.ListItems[t].tag));
      end;
    end;
...
begin
  if c is TComponent then begin
    var cc := c as TComponent;
    var name := cc.name;
    var val := ControlValueAsString(c);
    nvpl.AutoAdd := true;
    nvpl[name].Value := val;
  end;
end;
