...
{$ENDIF}
  ...
  TRDTPCallback = class;
  TRDTPProgressEvent = procedure (sMessage: string; sDebugLog: string; iPos, iMax: int64) of object;
  ...
  protected
    ...
    function Transact2(inPacket: TRDTPPacket; var outPacket: TRDTPPacket; sender: TObject; iTimeOutMS: int64 = 900000; bForget: boolean  = false):boolean;
  ...
  public
    ...
    function Transact(var Packet: TRDTPPacket; iTimeOutMS: int64 = -1; slDebug: TStringList = nil; bForget: boolean = false): boolean;overload;
    ...
    procedure SetLastError(iCode: int64; s: string);overload;
...
function TGenericRDTPClient.GetLastErrorCode: int64;
begin
  RESULT := 0;
end;
...
procedure TGenericRDTPClient.SetLastError(iCode: int64; s: string);
begin
  FLastError := s;
end;
...
function TGenericRDTPClient.Transact(var Packet: TRDTPPacket; iTimeOutMS: int64 = -1; slDebug: TStringList = nil; bForget: boolean = false): boolean;
...
//    while not bLegitResponse do begin
      ...
      iBytesRead := 0;
      header_read_count := 0;
      while iBytesRead < PACKET_HEADER_SIZE do begin
        if FConnection = nil then
          raise ETransportError.create('Connection dropped unexpectedly');
        ...
        iBytesThisREad := FConnection.ReadData(pcHeader+iBytesRead, PACKET_HEADER_SIZE-iBytesRead);
        ...
        if header_read_count > PACKET_HEADER_SIZE then
          raise Ecritical.create('transport layer error! Header Read took more than '+PACKET_HEADER_SIZE.ToString()+' iterations!');
      end;

      //Get info on packet
      //Extract length from header
      iLength := PInt64(pbyte(pcHeader)+PACKET_ADDRESS_PACKED_LENGTH)^;
...
function TGenericRDTPClient.Transact2(inPacket: TRDTPPacket; var outPacket: TRDTPPacket; sender: TObject; iTimeOutMS: int64 = 900000; bForget: boolean  = false):boolean;
var
  buffer : PByte;
  length : int64;
  pcHeader: PByte;
...
begin
  ...
  {$ENDIF}
        ...
        FConnection.WaitForData(Timeout);
        Fconnection.ReadData(pcHeader, PACKET_HEADER_SIZE, true);
        ...
        if iMarker <> PACKET_MARKER then begin
          ...
          raise ETransportError.create('packet marker invalid found '+memorytohex(pcHeader, PACKET_HEADER_SIZE));
        end;
